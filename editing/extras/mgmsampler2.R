  ...
  
{
  ...
  exo <- ifelse(is.null(exogenous), 0, exogenous)
  ...
  for(i in 1:n_order) {
    ...
    if(!is.null(factors[[i]])) {
      ...
      symflag <- mgm:::FlagSymmetric(factors[[i]])
  ...
  for(case in 1:N) {
    ...
    X[[case]] <- lapply(1:p, function(z) numeric(nIter))
    # Loop over iterations of the Gibbs sampler
    for(iter in 2:nIter) {

      # Looping over variables
      for(v in 1:p) {
        #X[[case]][[v]] <- numeric(nIter)
        ...
        if(type[v] != 'c') {
          ...
          for(ord in 1:n_order) {
            ...
            if(!is.null(n_rows) & !v %in% exo) { # only loop over factors if there are factors for given ord
              for(row in 1:n_rows) {
          ...
          X[[case]][[v]][iter] <- natPar <- sum(unlist(l_natPar))
